...
-- Tue Jun 16 23:19:37 2020
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ER_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';
...
CREATE TABLE IF NOT EXISTS `bookcity`.`Factura` (
  ...
  CONSTRAINT `fk_Factura_Domicilio1`
    ...
    REFERENCES `bookcity`.`Domicilio` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Factura_Libreria1`
    FOREIGN KEY (`Libreria_nit`)
    REFERENCES `bookcity`.`Libreria` (`nit`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE INDEX `fk_Factura_Domicilio1_idx` ON `bookcity`.`Factura` (`Domicilio_id` ASC);

CREATE INDEX `fk_Factura_Libreria1_idx` ON `bookcity`.`Factura` (`Libreria_nit` ASC);

CREATE INDEX `fk_Factura_Usuario1_idx` ON `bookcity`.`Factura` (`Usuario_cedula` ASC);
...
CREATE TABLE IF NOT EXISTS `bookcity`.`Factura_has_Libro` (
  ...
  CONSTRAINT `fk_Factura_has_Libro_Factura1`
    ...
    REFERENCES `bookcity`.`Factura` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE INDEX `fk_Factura_has_Libro_Libro1_idx` ON `bookcity`.`Factura_has_Libro` (`Libro_isbn` ASC);

CREATE INDEX `fk_Factura_has_Libro_Factura1_idx` ON `bookcity`.`Factura_has_Libro` (`Factura_id` ASC);
